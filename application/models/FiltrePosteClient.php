...
class Genius_Model_FiltrePosteClient
{

    public static function getTerminaux() {
        global $db;
        $sql = " SELECT * FROM ec_filtres_client  ";
    ...
    public function select($id = null){
        ...
        $sql = $db
            ->select()
            ->from('ec_filtres_client')
            ->join('ec_products_multilingual', 'ec_filtres_client.product_m_id = ec_products_multilingual.id',
                [
            ...
            ->where('ec_filtres_client.visible = 1')
            ->order('ec_filtres_client.stock DESC')
            ->order('ec_filtres_client.top DESC')
            ->order('ec_filtres_client.pertinence DESC')
        ;

        if($id <> null )
        {
            $sql = $sql->where("ec_filtres_client .product_id = $id");
        }
