...
class Genius_Model_FiltreTerminalPda
{

    public static function getTerminaux() {
        global $db;
        $sql = " SELECT * FROM ec_filtres_pda  ";
    ...
    public function select( $id = null){
        ...
        $sql = $db
            ->select()
            ->from('ec_filtres_pda')
            ->join('ec_products_multilingual', 'ec_filtres_pda.product_m_id = ec_products_multilingual.id',
                [
            ...
            ->where('ec_filtres_pda.visible = 1')
            ->order('ec_filtres_pda.stock DESC')
            ->order('ec_filtres_pda.top DESC')
            ->order('ec_filtres_pda.pertinence DESC')
        ;

        if($id <> null )
        {
            $sql = $sql->where("ec_filtres_pda .product_id = $id");
        }
