 ...
 * Time: 10:51
 */
class Genius_Model_FiltreEtiquetteBadgeuse
{

    public static function getAll() {
        global $db;
        $sql = " SELECT * FROM ec_filtre_badgeuses  ";
    ...
    public static function findArt($id) {
        ...
        $sql = $db
            ->select()
            ->from('ec_filtre_badgeuses')
            ->where("ec_filtre_badgeuses.product_id = $id")
        ;
        return $sql;
    }
    ...
    public function select($id=null){
        ...
        $sql = $db
            ->select()
            ->from('ec_filtre_badgeuses')
            ->join('ec_products_multilingual', 'ec_filtre_badgeuses.product_m_id = ec_products_multilingual.id',
                [
